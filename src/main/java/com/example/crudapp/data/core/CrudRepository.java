...
public class CrudRepository<T extends BaseEntity> {
    ...
    public List<T> findAll(int offset, int limit, Map<String, List<String>> queryParams, String sortParam, Class<?> dtoClass) {
        String tenantId = getActiveTenantId();
        ...
        if (dtoClass != null) {
            try {
                ...
                for (java.lang.reflect.Field f : com.example.crudapp.infrastructure.mapping.ReflectionCache.getDeclaredFields(dtoClass)) {
                    String name = f.getName();
                    Class<?> searchClass = entityClass;
                    boolean found = false;
                    while (searchClass != null) {
                        for (java.lang.reflect.Field ef : com.example.crudapp.infrastructure.mapping.ReflectionCache.getDeclaredFields(searchClass)) {
