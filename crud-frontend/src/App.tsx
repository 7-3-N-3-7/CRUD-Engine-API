...
const DashboardContent: React.FC = () => {
  ...
  // While the OIDC library is initialising or redirecting to Keycloak, show loading screen
  if (auth.isLoading || (!auth.isAuthenticated && !auth.error)) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center flex-col gap-3">
        <div className="text-blue-400 animate-pulse text-xl font-medium">Redirecting to Keycloak Login…</div>
        <div className="text-slate-500 text-sm">Authentication is mandatory for all access.</div>
      </div>
    );
  }
  ...
  if (auth.error) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center flex-col gap-4 p-4 text-center">
        <div className="text-red-400 text-xl font-semibold">Authentication Error</div>
        <div className="text-slate-300 max-w-md bg-slate-800 p-4 rounded-lg border border-slate-700 text-sm">
          {auth.error.message}
        </div>
        <button
          onClick={handleLogin}
